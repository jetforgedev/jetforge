 ...
 *   cd backend
 *   node scripts/backfillVolumeBuckets.js [lookbackHours]
 *
 * Default lookback is 48h.
 */
...
function getMinuteBucketStart(date) {
...
async function main() {
  ...
  for (let i = 0; i < buckets.length; i += BATCH) {
    ...
    await Promise.all(
      batch.map((b) =>
        prisma.tradeVolumeBucket.upsert({
    ...
    console.log(`  batch ${Math.floor(i / BATCH) + 1}/${Math.ceil(buckets.length / BATCH)} done`);
  }
  ...
  for (const mint of affectedMints) {
    const sum = await prisma.tradeVolumeBucket.aggregate({

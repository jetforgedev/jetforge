...
// Returns exactly ONE momentum state label — never two conflicting states
function getMomentum(trades5m: number, trades30m: number): { label: string; badge: string; color: string; borderColor: string } | null {
  if (trades5m >= 3 && trades30m > 0 && (trades5m / trades30m) >= 0.4) {
    return { label: "🔥 Hot",    badge: "Hot",    color: "#ffb347", borderColor: "#ff8c0030" };
  }
  if (trades30m > 0 && trades5m === 0) {
    return { label: "💤 Cooling", badge: "Cooling", color: "#666",    borderColor: "#55555530" };
  }
  if (trades5m > 0) {
    return { label: "🟢 Stable",  badge: "Stable",  color: "#00cc77", borderColor: "#00cc7730" };
  }
  return null;
}
...
export function TradesList({ mint, symbol }: TradesListProps) {
  ...
  if (isLoading) {
    return (
      <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-4">
        <div className="text-sm text-white font-semibold mb-4">Recent Trades</div>
  ...
  const momentum = getMomentum(trades5m, trades30m);
        ...
        <div className="flex items-center gap-2">
          ...
          {momentum && (
            <span
              ...
              style={{ color: momentum.color, borderColor: momentum.borderColor, background: momentum.borderColor }}
            >
              {momentum.label}
            </span>
          )}
      ...
      {trades.length > 0 && (
        <div className="px-4 py-2.5 border-b border-[#1a1a1a]">
          <div className="flex justify-between text-[10px] mb-1.5">
            ...
            <span className="text-[#ff4444] font-semibold">{sellPct.toFixed(0)}% SELL</span>
          </div>
          <div className="h-2 rounded-full overflow-hidden flex">
            <div className="h-full bg-[#00ff88] transition-all duration-500" style={{ width: `${buyPct}%` }} />
            <div className="h-full bg-[#ff4444] transition-all duration-500 flex-1" />
          </div>
        </div>
      )}
